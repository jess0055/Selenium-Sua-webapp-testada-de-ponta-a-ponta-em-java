...
public class NovoLeilaoPage {
	...
	public void preenche(String nome, double valor, String usuario, boolean usado) {
		...
		txtValor.sendKeys(String.valueOf(valor));//sendkeys recebe string/converter valor
		
		Select cbUsuario = new Select(driver.findElement(By.name("leilao.usuario.id")));//select no lugar do sendkeys
		cbUsuario.selectByVisibleText(usuario);//procura  usuario dentro do combo
